<?php
/**
* Extension for the default TMF Colorparser class by oorf|fuckfish  (fish@stabb.de)
* This extension adds support for remoteCP 4.0.3.5
*/
class rcp_textparser extends TMFColorParser
{
	public function __construct()
	{
		$this->autoContrastColor('#ffffff');
	}

	/**
	* Returns a string without any tmcode
	*
	* @param int $str
	* @author hal.sascha
	*/
	public function stripCode($str)
	{
		$pattern = '/(\$[lhLH]{1}\[(.*?)\]|\$[hilswnmogztHILSWNMGZOT]{1}|\$[\d\a-fA-F]{3})/';
		return preg_replace($pattern, '', $str);
	}

	/**
	* Returns a string without the tags from the tags array
	*
	* @param int $str
	* @param array $tags
	* @author hal.sascha
	*/
	public function stripTags($str, $tags)
	{
		...
		if(is_array($tags)) {
			...
			$tags = $tags.strtoupper($tags);
		}

		//Build pattern
		$pattern  = '/(';
		$pattern .= ($removeL) ? '\$[lhLH]{1}\[(.*?)\]|' : '';
		$pattern .= '\$['.$tags.']{1}';
		$pattern .= ($removeC) ? '|\$[\d\a-fA-F]{3}' : '';
		$pattern .= ')/';
		return preg_replace($pattern, '', $str);
	}

	/**
	* Returns a tmcode formated string as HTML code
	*
	* @param int $str
	...
	* @author hal.sascha
	*/
	public function toHTML($str, $stripColors = false, $stripLinks = false, $stripTags = '')
	{
		if(is_null($stripColors) || Core::getObject('session')->admin->nocode) {
			return $this->stripCode($str);
		}
		return parent::toHTML($str, $stripColors, $stripLinks, $stripTags);
	}

	/**
	* Returns a formated racetime
	*
	* @param int $time
	* @author hal.sascha
	*/
	public function toRaceTime($time, $splittime = false)
	{
		//Return string if time is empty
		if($time == 0)
			return (Core::getObject('status')->gameinfo['GameMode'] == 4) ? '------' : '--:--.--';

		//Return time without format if gamemode is stunts
		...
		if($splittime) {
			...
			$time .= ($min) ? ':' : '';
			$time .= sprintf('%02d',$sek);
			$time .= '.';
			$time .= sprintf('%02d',substr($hsek, 0, -1));
		} else {
			$time = sprintf('%02d',$min).':'.sprintf('%02d',$sek).'.'.sprintf('%02d',substr($hsek, 0, -1));
		}
...
/**
 * TMFColorParser v1.3 by oorf|fuckfish (fish@stabb.de)
 *
 * Change by hal.ko.sascha: replaced all count() calls inside the for loops
 */
